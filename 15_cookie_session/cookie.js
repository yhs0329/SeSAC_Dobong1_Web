...
const cookieConfig = {
  /*
    ...
    - httpOnly : http 통신망 접근 허용
    - path : '/abc', // 쿠키가 해당 경로와 그 하위 경로에서만 활성화
        (default: '/')
    - secure : https 로 통신하는 경우메나 쿠키 전송
...
// 쿠키 가져오기
// req.cookies >> 쿠기 정보 담겨 있음
app.get("/getCookie", (req, res) => {
  //   console.log(req.cookies);
  //   res.send(req.cookies); // 암호화 되지 않은 쿠키일 때
  console.log(req.signedCookies); // 암호화 된 쿠키일 때
  res.send(req.signedCookies);
});

// 쿠키 삭제
// res.clearCookie(쿠키 이름, 쿠키값, 옵션)
app.get("/clearCookie", (req, res) => {
  res.clearCookie("myCookie", "cookie~~", cookieConfig);
  // res.cookie로 설정했던 인자와 똑같이 써준다.
  res.send("쿠키 삭제");
});
