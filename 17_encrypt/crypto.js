const crypto = require("crypto"); // node.js 내장 모듈, 설치x
/*
1. crypto를 이용한 단방향 암호화 구현 - 복호화 불가능
    - createHash(알고리즘)
    - pbkdf25Sync(비밀번호, 솔트, 반복횟수, 키의 길이, 알고리즘)
*/

// 1-1) createHash(알고리즘).update(암호화할 값).digest(인코딩)
// 인코딩으로는 base64, hex, ascii, binary 등 쓸 수 있음

// 비밀번호를 받아서 암호화 사켜주는 함수
...
// 1-2) pbkdf2Sync(암호화할 값, 솔트, 반복횟수, 키 길이, 알고리즘).toString(인코딩 방식)
/*
- 솔트를 이용해서 해싱하는 함수
- salt 생성 : crypto.randomBytes(바이트 수)
- randomBytesdhk pbkdf25Sync 함수는 버퍼 형식의 데이터를 리턴
*/
...
function encrypt(text) {
  ...
  // 2. 실제 데이터 암호화 작업
  // let encrypted = cipher.update(암호화할 데이터, 입력 인코딩, 출력 인코팅)
